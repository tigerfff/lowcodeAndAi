    ...
    data.forEach((item, index) => {
      if (states.selectable) {
        if (
          ...
          toggleRowSelection(selection, item, value)
        ) {
...
TableStore.prototype.getTableRowsHeight = function() {
  ...
  if (!this.table.$ready) return Vue.nextTick(() => this.getTableRowsHeight());
  if (this.table.isVirtualScroll && process.env.NODE_ENV === 'production') {
    // 虚拟滚动在生产环境下只取第一行高度来得到行高以减少性能消耗
    // 开发环境下取所有行的高度，且行高不固定时做控制台的提示
    const row = this.table.bodyWrapper.querySelector(
    ...
    const rows = this.table.bodyWrapper.querySelectorAll(
      'tbody > tr.el-table__row'
    );
...
TableStore.prototype.getRowHeight = function() {
  ...
  if (this.table.isVirtualScroll && !isRowsHeightEqual) {
    process.env.NODE_ENV !== 'production' &&
      console.error(
        'HUI table control virtual scroll only supports fixed height, and there are different row height in use, please correct!'
      );
  }
  return distinct[0];
};
...
TableStore.prototype.updateColumns = function() {
  ...
  states.columns = []
    .concat(states.fixedLeafColumns)
    .concat(leafColumns)
    .concat(states.rightFixedLeafColumns);
...
TableStore.prototype.toggleRowSelection = function(
  row,
  selected,
  emitChange = true
) {
