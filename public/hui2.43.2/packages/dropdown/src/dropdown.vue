...
import ElButtonGroup from 'hui/packages/button-group';
import { generateId } from 'hui/src/utils/util';
...
export default {
  ...
  render() {
    const { hide, clickOutside, splitButton, type, dropdownSize } = this;
    ...
    const triggerElm = !splitButton ? (
      ...
      <el-button-group>
        ...
        >
          <i class='el-dropdown__icon h-icon-angle_down_sm' />
        </el-button>
      </el-button-group>
    );

    return (
      <div class='el-dropdown' v-clickoutside={clickOutside}>
        {triggerElm}
        {this.$slots.dropdown}
      </div>
    );
